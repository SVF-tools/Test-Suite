...
void *foo(void *x) {
  ...
  INTERLEV_ACCESS(4,"cs1.Call,cs6.foo2,cs5.foo","0,3,4,2:");  
  return x;
}
...
void *foo2(void *x) {
  ...
  Global ++;
  INTERLEV_ACCESS(3,"cs1.Call,cs6.foo2","0,3,2");  

cs5: pthread_create(&t4, NULL, foo, NULL);

     Global ++;
     INTERLEV_ACCESS(3,"cs1.Call,cs6.foo2","0,3,4,2");  
...
cs6: pthread_create(&t3, NULL, foo2, NULL);
     ...
     Global ++;
     INTERLEV_ACCESS(0,"cs1.Call","0,3,4,2");  
